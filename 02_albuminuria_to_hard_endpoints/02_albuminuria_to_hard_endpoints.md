...
> 🔬 **因果中介專論**：UACR＋SBP 中介 finerenone CV 益處約一半、以及『未被捕捉的另一半』，見獨立檔 [`uacr_sbp_mediation_of_cv_benefit.md`](uacr_sbp_mediation_of_cv_benefit.md)（血鉀『不』中介的反面見主題五 potassium_mediation_of_cv_benefit.md）。
...
概念呈現(非本地實測曲線):在 FIDELITY,達 ≥30% UACR 早期降幅者(finerenone 組 53.2% vs placebo 27.0%)後續 kidney composite 累積發生率較低——因 UACR 變化 mediated kidney 效應達 64–84% [📌 albuminuria_mediation_Agarwal_2023]。**警示**:non-responder 不等於無效,因 CV 益處僅 26–37% 經 UACR 中介 [📌 albuminuria_mediation_Agarwal_2023],尚有 SBP 與未知路徑 [📄 albuminuria_mediation_Agarwal_2026]。
...
*註:R²=0.97 為 Inker 2023 對 total slope 的 trial-level 關聯 [📄 egfr_slope_surrogate_Inker_2023];mediation 百分比見 §4.1(📌 abstract,Agarwal 2023)與 §4.2(📄 本地全文,Agarwal 2026)。*
...
2. **時間性弔詭**:Agarwal 2026 指出 CV 益處在數月內出現、**早於可測得的腎保護**(腎保護約需 18 個月才具統計顯著性),提示以「4 個月 mediator → 後續事件」的框架可能低估或錯配真實時序 [📄 albuminuria_mediation_Agarwal_2026]。
...
- **Mediation-derived(詮釋須保留)**:Agarwal 2023 與 Mc 2025 仍僅取得結構化 abstract,數字未經全文交叉驗證 [📌 albuminuria_mediation_Agarwal_2023] [📌 albuminuria_mediation_Mc_2025];Agarwal 2026(FIDELITY UACR+SBP mediation)本輪已取得本地全文,數字已 grep 交叉驗證 [📄 albuminuria_mediation_Agarwal_2026]。
...
- **Mc Causland 2025 FINEARTS-HF(NDT abstract #3019)、Heerspink 2025 Nat Med albuminuria surrogate** —— 均為 abstract-only 或付費牆。(**Agarwal 2026 NDT** 本輪已由使用者上傳本地 PDF 經 LlamaParse 轉檔取得全文,§4.2 相關數字已 grep 交叉驗證並升級為 📄。)
