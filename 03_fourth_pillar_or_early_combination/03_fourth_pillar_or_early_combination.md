...
ADA/KDIGO 2022 consensus 則確立了 **layered therapy(疊層治療)** 的框架:在健康生活型態的地基上,「疊上」已證實能改善腎臟與心血管硬終點的藥物,並選加達成血糖、血壓、血脂中介目標的治療;共識明言 finerenone 與 SGLT2i、GLP-1RA 的效益「based on preclinical studies 應為 additive,但仍需臨床研究」[guideline_position_de_2022]。這句話正是本主題的核心張力:**「additive」是被假設的,而非被 head-to-head 證實的**。
...
> **證據層級誠實標示**:上述 subgroup/post-hoc 皆為**探索性、非為交互作用而 power**,且 FIDELITY 的 SGLT2i/GLP-1RA 使用者基線腎功能較好(confounding by indication),CONFIDENCE 的隨機化才是唯一無此偏誤的設計 [hyperkalemia_combo_Rossing_2022][hyperkalemia_combo_Sinclair_2026]。
...
2. **「同步起始」目前是 surrogate-based 的 evidence expansion,不是 guideline routine**。KDIGO 2024 仍給 finerenone 2A、且情境為「加在 RASi±SGLT2i 之上」;要主張「同步起始成為普遍標準」,仍需 longer-term hard-outcome confirmation [note_kdigo2024][hyperkalemia_combo_Wen_2026]。
...
> **證據性質標示**:綠色路徑中「同步起始壓 UACR」為 **RCT surrogate 證實**(CONFIDENCE);「SGLT2i 先行以降鉀」為 **hypothesis-generating**(Sinclair/FIVE-STAR);三藥硬終點益處為 **模型與 subgroup 外推**(Neuen/Singh),尚待 outcome trial。加 finerenone 於 RASi±SGLT2i 之上、以 UACR 導引優化,則已是 **guideline routine**(KDIGO 2024 / ADA-KDIGO consensus)[note_kdigo2024][guideline_position_de_2022]。
