...
| Paper | 內容 | Reason |
|-------|------|--------|
| `stage4_ckd_Bakris_2023`（Kidney Int, FIDELITY 腎臟探索性分析） | 摘要已含 HR/NNT 關鍵數字，經 Europe PMC + PubMed 核對後採用 | 全文付費牆 |
| `asian_subgroup_Li_2024`（Cardiorenal Med review） | — | 訂閱牆 |
| `baseline_goals_absolute_Zuin_2026`（fragility 分析） | fragility index 細節未取得，故未斷言 | OUP 付費牆 |
| `baseline_goals_absolute_Zhao_2026`（胰島素阻抗, n=45 單中心） | 僅列為機轉假說 | 小樣本真實世界 |
| `r2_Bakris_2015`（ARTS-DN）/ `r2_Solomon_2024`（FINEARTS-HF）/ `r2_Agarwal_2025`（CONFIDENCE NEJM） | 僅引摘要層數字 | 付費牆 |
