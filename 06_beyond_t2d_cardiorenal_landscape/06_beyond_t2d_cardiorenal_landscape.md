...
這三份構成的敘事非常清楚:**在具白蛋白尿的 T2D-CKD、且已在 RAS 阻斷背景上,finerenone 同時降低腎臟進展與 HHF**。此一結論已屬 **guideline routine**——多數多專科指引強推,KDIGO 給予 2A 級建議,並要求疊加於最大耐受 RASi(±SGLT2i)之上[fine_heart_pooled_Vaduganathan_2024.md] 📄。
...
結果:total eGFR slope −3.50 vs −4.23 mL/min/1.73m²/年(**差 0.73,95% CI 0.22–1.24**);**month 12 白蛋白尿降 42%(95% CI 35%–48%)**;kidney failure 或 ≥40% eGFR 下降為 7.42 vs 9.60 events/100 patient-years(**HR 0.74,95% CI 0.57–0.97**)[glomerular_subgroup_Neuen_2026.md] 📌。
...
| **T1D-CKD** | FINE-ONE | 242 | **surrogate**(6mo UACR) | −25% UACR;高血鉀 10.1% vs 3.3% | surrogate-based | 🟡 FDA 受理 Priority Review,尚未核准 |
...
```mermaid
graph TD
    A["MR 過度活化<br/>發炎 + 纖維化<br/>(跨器官共同路徑)"] --> B["T2D-CKD 地基<br/>FIDELIO / FIGARO / FIDELITY<br/>硬性終點・已核准"]
    B --> C["HFpEF/HFmrEF LVEF≥40%<br/>FINEARTS-HF・已核准 2025"]
    B --> D["T1D-CKD<br/>FINE-ONE・UACR surrogate<br/>FDA Priority Review 受理中"]
    B --> E["非糖尿病 CKD<br/>FIND-CKD・eGFR slope surrogate<br/>尚無獨立適應症"]
    E --> F["Glomerular disease 亞群<br/>IgAN/FSGS/MN・exploratory<br/>hypothesis-generating"]
    C --> G["整體 CKM platform 概念<br/>FINE-HEART 18,991<br/>INFINITY 14,574<br/>全因死亡↓ HHF↓ 腎↓ AF↓ 猝死↓"]
    D --> G
    E --> G
    G -.->|"仍待驗證的邊界"| H["CV 死亡 pooled 未達顯著<br/>HF 族群腎複合未降(acute dip)<br/>多為 surrogate / exploratory"]

    classDef approved fill:#1b5e20,stroke:#0d3311,color:#fff;
    classDef expansion fill:#e65100,stroke:#8f3200,color:#fff;
    classDef caution fill:#7f1d1d,stroke:#4c0f0f,color:#fff;
    class B,C approved;
    class D,E,F,G expansion;
    class H caution;
```
